...
class RNN:
    ...
    def _get_cell(hidden_size, cell_type):
        if cell_type == "vanilla":
            ...
            return tf.contrib.rnn.MultiRNNCell([tf.nn.rnn_cell.BasicLSTMCell(n) for n in num_units])
            #return tf.nn.rnn_cell.BasicLSTMCell(hidden_size)
        elif cell_type == "gru":
            num_units = [256,128,64]
            return tf.contrib.rnn.MultiRNNCell([tf.nn.rnn_cell.GRUCell(n) for n in num_units])
